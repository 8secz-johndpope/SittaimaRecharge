...
export default class Print extends React.Component {
		...
		this.onPrint = () => {
			...
			if (isIframe) {
				if (clearIframeCache) {
					...
					// 单例模式缓存模式
					if (this.changed || this.iframe !== singletonCacheData.iframe) {
	...
	writeTemplate = doc => {
		...
		if (lazyRender) {
			doc.write(
				`<html><head></head><body ${bodyAttrs}><div></div></body></html>`
			);
			doc.head.innerHTML = this.getHead();
			ReactDOM.render(
				this.renderChild(),
				doc.body.getElementsByTagName("div")[0]
			); // React的未来版本可能会异步地呈现组件
			if (bodyStyle) {
